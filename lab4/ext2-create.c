...
void write_inode_table(int fd) {
	...
	struct ext2_inode root_directory_node = {0};

	root_directory_node.i_mode = EXT2_S_IFDIR
								  | EXT2_S_IRUSR
								  | EXT2_S_IWUSR
								  | EXT2_S_IXUSR
								  | EXT2_S_IRGRP
	...
	root_directory_node.i_size = 1024;
	root_directory_node.i_uid = 0;
	root_directory_node.i_gid = 0;
	root_directory_node.i_atime = current_time;
	root_directory_node.i_ctime = current_time;
	root_directory_node.i_mtime = current_time;
	root_directory_node.i_dtime = 0;
	root_directory_node.i_links_count = 3;
	root_directory_node.i_blocks = 2;
	root_directory_node.i_block[0] = ROOT_DIR_BLOCKNO;
	write_inode(fd, EXT2_ROOT_INO, &root_directory_node);
	...
	struct ext2_inode sym_link = {0};
	sym_link.i_mode = EXT2_S_IFLNK
						 | EXT2_S_IRUSR
						 | EXT2_S_IWUSR
						 | EXT2_S_IRGRP
						 | EXT2_S_IROTH;
	...
	sym_link.i_uid = 1000;
	sym_link.i_gid = 1000;
	sym_link.i_atime = current_time;
	sym_link.i_ctime = current_time;
	sym_link.i_mtime = current_time;
	sym_link.i_dtime = 0;
	sym_link.i_links_count = 1;
	sym_link.i_blocks = 0;
	char file_path[] = "hello-world";
	memcpy((char*)sym_link.i_block, file_path, 12);
	write_inode(fd, HELLO_INO, &sym_link);
}
